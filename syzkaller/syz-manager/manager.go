...
type Manager struct {
	...
	// gwangmu: per-function weight map
	// filename -> funcname -> weight
	weightMap map[string]map[string]uint32
}
...
func RunManager(cfg *mgrconfig.Config) {
	...
	mgr := &Manager{
		...
		weightMap:        make(map[string]map[string]uint32),
	}

	mgr.loadWeightMap() // gwangmu: Load weight map.
	mgr.preloadCorpus()
...
func (mgr *Manager) loadWeightMap() {
	log.Logf(0, "loading weight map...")
	...
	witems, err := ioutil.ReadFile(mgr.cfg.WeightMap)
	if err != nil {
		log.Logf(0, "failed to load weight map")
		return
	}

	var wmap []WeightItem
	if _err := json.Unmarshal(witems, &wmap); _err != nil {
		log.Logf(0, "failed to unmarshal weight map: %v", _err)
		return
	}

	for _, item := range wmap {
		item.Weight *= 100000 // gwangmu: just did it for precision
		if _, ok := mgr.weightMap[item.File]; !ok {
			mgr.weightMap[item.File] = make(map[string]uint32)
		}
		if prevW, ok := mgr.weightMap[item.File][item.Func]; ok {
			log.Logf(0, "weight already exists (file: %v, func: %v). Taking max...",
				item.File, item.Func)
			if float64(prevW) > item.Weight {
				item.Weight = float64(prevW)
			}
		}
		mgr.weightMap[item.File][item.Func] = uint32(item.Weight)
	}
}
...
func (mgr *Manager) saveCrash(crash *Crash) bool {
	...
	// gwangmu: write earliest time
  efile_name := filepath.Join(dir, "earliest_time")
	if _, err := os.Stat(efile_name); err != nil {
		ts := time.Now().Format("2006/01/02 15:04:05 ")
		osutil.WriteFile(efile_name, []byte(ts))
	}
...
// gwangmu: calculate and save the input weight.
func (mgr *Manager) calcInputWeight(inp rpctype.Input, cov []uint32) uint32 {
	sig := hash.String(inp.Prog)
	log.Logf(5, "(gwangmu) calculating input weight...")
	rg, err := getReportGenerator(mgr.cfg, mgr.serv.modules)
	if err != nil {
		log.Logf(0, "(gwangmu) cannot get report generator for weight calculation.")
		return 0
	}
	...
	rgProg := cover.Prog{
		...
		PCs:  coverToPCs(rg, cov),
	}

	files, err := cover.DumpOneFunctionHitMap(rg, rgProg, mgr.coverFilter)
	if err != nil {
		log.Logf(0, "(gwangmu) cannot prepare function hit map.")
		return 0
	}

	inputWeight := uint32(0)
	nFuncs := uint32(0)
	for filename, file := range files {
		for _, function := range file.Functions {
			nFuncs += uint32(function.Covered)
			if _, ok := mgr.weightMap[filename]; ok {
				if weight, ok := mgr.weightMap[filename][function.Name]; ok {
					inputWeight += weight * uint32(function.Covered)
				}
			}
		}
	}
	...
	if nFuncs <= 0 {
		...
		return inputWeight / nFuncs
	}
}

func (mgr *Manager) newInput(inp rpctype.Input, sign signal.Signal) (uint32, bool) {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()
	if mgr.saturatedCalls[inp.Call] {
		return mgr.calcInputWeight(inp, inp.Cover), false
	}
	...
	sig := hash.String(inp.Prog)
	//mgr.printFunctionHitMap(sig, inp, "0") // gwangmu: only for testing
	...
	if old, ok := mgr.corpus[sig]; ok {
		...
		// gwangmu: weight from merged cov.
		old.Weight = mgr.calcInputWeight(inp, old.Cover)
		mgr.corpus[sig] = old
	} else {
		mgr.corpus[sig] = CorpusItem{
			...
			Weight:  mgr.calcInputWeight(inp, inp.Cover),
		}
	...
	return mgr.corpus[sig].Weight, true
}
...
// gwangmu
func (mgr *Manager) getInputWeight(sig string) uint32 {
	if ci, ok := mgr.corpus[sig]; ok {
		return ci.Weight
	} else {
		log.Fatalf("mgr.corpus has an input with an undetermined weight")
		return ^uint32(0)
	}
}

// gwangmu
func (mgr *Manager) printFunctionHitMap(sig string, inp rpctype.Input, postfix string) error {
	rg, err := getReportGenerator(mgr.cfg, mgr.serv.modules)
	if err != nil {
		log.Logf(3, "cannot print function trace.")
		return err
	}
	...
	files, err := cover.DumpOneFunctionHitMap(rg, rgProg, mgr.coverFilter)
	if err != nil {
		log.Logf(3, "cannot prepare file map.")
		return err
	}

	outpath := filepath.Join(mgr.cfg.Workdir, "funcs_"+postfix+".csv")
	f, err := os.Create(outpath)
	if err != nil {
		log.Logf(3, "cannot open print function file.")
		return err
	}
	defer f.Close()
	...
	var csvHeader = []string{"Filename", "Function", "Count", "Module"}
	if err := writer.Write(csvHeader); err != nil {
		log.Logf(3, "cannot write csv header.")
		return err
	}
	...
	for filename, file := range files {
		for _, function := range file.Functions {
			writer.Write([]string{filename,
				function.Name, fmt.Sprintf("%d", function.Covered), file.Module})
		}
	}
	return nil
}
