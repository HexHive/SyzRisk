...
type RPCManagerView interface {
	...
	calcInputWeight(inp rpctype.Input, cov []uint32) uint32
	newInput(inp rpctype.Input, sign signal.Signal) (uint32, bool)
  getInputWeight(sig string) uint32
...
func (serv *RPCServer) NewInput(a *rpctype.NewInputArgs, r *rpctype.NewInputRes) error {
  r.Weight = ^uint32(0)   // gwangmu: this shouldn't be returned.
	...
	if !genuine && !rotated {
    r.Weight = serv.mgr.calcInputWeight(a.Input, a.Input.Cover)
		return nil
	}
  if weight, ok := serv.mgr.newInput(a.Input, inputSignal); !ok {
    r.Weight = weight
		return nil
	} else {
    r.Weight = weight
...
/* gwangmu
func (serv *RPCServer) Weight(a *rpctype.WeightArgs, r *rpctype.WeightRes) error {
  r.Weight = serv.mgr.getInputWeight(a.Sig)
  log.Logf(0, fmt.Sprintf("(gwangmu) sending weight... (weight=%d)", r.Weight))
  return nil
}
*/

// gwangmu
func (serv *RPCServer) RemoteLog(a *rpctype.RemoteLogArgs, r *int) error {
  log.Logf(0, "(gwangmu) %v: %v", a.Index, a.Log)
  return nil
}
