...
func DumpOneFunctionHitMap(rg *ReportGenerator, prog Prog, coverFilter map[uint32]uint32) (map[string]*File, error) {
  progs := []Prog{ prog }
  ...
  for fname, file := range files {
    _file := &File{ Module: file.module }
    hit := false
    for _, function := range file.functions {
      if function.covered > 0 {
        hit = true
        _file.Functions = append(_file.Functions, &Function{
          Name:     function.name,
          PCs:      function.pcs,
