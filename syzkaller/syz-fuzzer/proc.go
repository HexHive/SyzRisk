...
func (proc *Proc) loop() {
	...
	for i := 0; ; i++ {
		...
		if item != nil {
			...
			case *WorkTriage:
        log.Logf(0, "(gwangmu) TRIAGING...")
				proc.triageInput(item)
			case *WorkCandidate:
        log.Logf(0, "(gwangmu) EXECUTING CANDIDATE...")
				proc.execute(proc.execOpts, item.p, item.flags, StatCandidate)
			case *WorkSmash:
        log.Logf(0, "(gwangmu) SMASHING...")
    ...
    log.Logf(0, "(gwangmu) GEN/MUT...")
      ...
      proc.fuzzer.rlog("mutating program... (weight: %d)", p.Weight)
...
func (proc *Proc) triageInput(item *WorkTriage) {
	...
	sig := hash.Hash(data)
  item.p.Weight = ^uint32(0)

	log.Logf(2, "added new input for %v to corpus:\n%s", logCallName, data)
  item.p.Weight = proc.fuzzer.sendInputToManager(rpctype.Input{
		Call:     callName,
		CallID:   item.call,
		Prog:     data,
...
func (proc *Proc) executeRaw(opts *ipc.ExecOpts, p *prog.Prog, stat Stat) *ipc.ProgInfo {
	...
	for try := 0; ; try++ {
		...
		if err != nil {
			...
			if try > 10 {
        proc.fuzzer.rlog("<< before SYZFATAL >> pid: %v, try: %v", proc.pid, try)
