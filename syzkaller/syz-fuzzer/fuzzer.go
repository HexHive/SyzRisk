...
// gwangmu
func (fuzzer *Fuzzer) rlog(format string, args ...interface{}) {
	a := &rpctype.RemoteLogArgs{
		Index: fuzzer.name,
		Log:   fmt.Sprintf(format, args...),
	}
	fuzzer.manager.Call("Manager.RemoteLog", a, nil)
}
...
func (fuzzer *Fuzzer) sendInputToManager(inp rpctype.Input) uint32 {
	a := &rpctype.NewInputArgs{
		Name:  fuzzer.name,
		Input: inp,
	}
	r := &rpctype.NewInputRes{}
	if err := fuzzer.manager.Call("Manager.NewInput", a, r); err != nil {
		log.Fatalf("Manager.NewInput call failed: %v", err)
	}
	//fuzzer.rlog("received weight (weight: %d)", r.Weight)
	return r.Weight
}

func (fuzzer *Fuzzer) addInputFromAnotherFuzzer(inp rpctype.Input) {
	p := fuzzer.deserializeInput(inp.Prog, inp.Weight)
	if p == nil {
		return
	}
	...
	log.Logf(0, "(gwangmu) added input from another fuzzer (weight=%d)", p.Weight)
}

func (fuzzer *Fuzzer) addCandidateInput(candidate rpctype.Candidate) {
	p := fuzzer.deserializeInput(candidate.Prog, ^uint32(0))
	if p == nil {
		return
	}
	flags := ProgCandidate
	...
	log.Logf(0, "(gwangmu) added candidate input (weight=%d)", p.Weight)
}

func (fuzzer *Fuzzer) deserializeInput(inp []byte, weight uint32) *prog.Prog {
...
func (fuzzer *FuzzerSnapshot) chooseProgram(r *rand.Rand) *prog.Prog {
	...
	if fuzzer.corpus[idx].Weight == ^uint32(0) {
		log.Fatalf("input with an unknown weight in the corpus")
	}
	return fuzzer.corpus[idx]
}

func (fuzzer *Fuzzer) calcPriority(base int64, weight uint32) int64 {
  // NOTE: we're gonna stick to 2^{<some_constant> * (2w-1)} - the terminal
  // priority of RGF [CCS'21] - because we're assuming an accumulated corpus,
  // we don't really need to care about exploration, and because inputs that
  // have already been in the corpus will be feeded to the candidate list,
  // they'll not be ignored by "big" weight inputs.
  if (weight == 0 || fuzzer.maxWeight == 0) {
    return base
  }
	w := float64(weight) / float64(fuzzer.maxWeight)
	return int64(float64(base) * math.Exp2(5 * (2*w-1)))  // NOTE: 3 --> 7
	//return int64(float64(base) * (1 + 127*w))
}

func (fuzzer *Fuzzer) addInputToCorpus(p *prog.Prog, sign signal.Signal, sig hash.Sig) {
	fuzzer.corpusMu.Lock()
	if fuzzer.maxWeight < p.Weight {
		fuzzer.rlog("(gwangmu) recalculating priorities... (maxW: %d)", p.Weight)
		fuzzer.maxWeight = p.Weight
		fuzzer.sumPrios = 0
		for i, pp := range fuzzer.corpus {
			fuzzer.sumPrios += fuzzer.calcPriority(fuzzer.corpusBasePrios[i], pp.Weight)
			fuzzer.corpusPrios[i] = fuzzer.sumPrios
		}
	}
	if _, ok := fuzzer.corpusHashes[sig]; !ok {
		fuzzer.corpus = append(fuzzer.corpus, p)
		fuzzer.corpusHashes[sig] = struct{}{}
		basePrio := int64(len(sign))
		if sign.Empty() {
			basePrio = 1
		}
		fuzzer.corpusBasePrios = append(fuzzer.corpusBasePrios, basePrio)
		prio := fuzzer.calcPriority(basePrio, p.Weight)
