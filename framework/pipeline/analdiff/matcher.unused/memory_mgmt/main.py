...
SHORT_NAME='memory_mgmt'
DESCRIPTION='Find any memory management calls.'
...
def OnDiffLine(line, scope_type, scope_name, diff_type):
    if (scope_type == 'func'):
        ...
        free_re = re.search('(delete|free|unregister)\w*\(', line)
        alloc_re = None
        if (diff_type == '+'):
            alloc_re = re.search('(new|alloc|register)\w*\(', line)
        if (free_re or alloc_re):
            log.SPECIAL(SHORT_NAME + ": found memory management call: {}".format(line))
            global current_commit_taken
